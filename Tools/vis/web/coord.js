// 表单坐标 (X前/Y右/Z上, m) <-> UE 坐标 (X前/Y右/Z上, cm): 仅单位换算, 无轴交换。
// 服务端 display 用 coords.py::to_web 做 UE->three.js 的独立轴变换, 与此无关。
function vecToUE(web) {
  if (!Array.isArray(web) || web.length < 3) return [0, 0, 0];
  return [web[0] * 100, web[1] * 100, web[2] * 100];
}

function vecToWeb(ue) {
  if (!Array.isArray(ue) || ue.length < 3) return [0, 0, 0];
  return [ue[0] / 100, ue[1] / 100, ue[2] / 100];
}

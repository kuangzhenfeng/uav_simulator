...
│  │  PID 反馈控制                                             │  │
│  │  • 比例项 (Kp * error)                                    │  │
│  │  • 阻尼项 (Kd * angular_velocity)                         │  │
│  └──────────────────────────────────────────────────────────┘  │
│                              +                                   │
│  ┌──────────────────────────────────────────────────────────┐  │
│  │  前馈控制 (Feedforward) [可选，默认启用]                  │  │
│  │  • τ_ff = I * α_desired                                   │  │
│  │  • 减少反馈延迟，提升响应速度 20-30%                      │  │
│  └──────────────────────────────────────────────────────────┘  │
│                              +                                   │
│  ┌──────────────────────────────────────────────────────────┐  │
│  │  自适应控制 (Adaptive) [可选，默认关闭]                   │  │
│  │  • MIT规则：d̂ += γ * e * dt                              │  │
│  │  • 遗忘因子：d̂ *= λ                                       │  │
│  │  • 补偿模型误差和外部扰动                                 │  │
│  └──────────────────────────────────────────────────────────┘  │
...
**关键特性**：
- **分层控制**：NMPC规划层 → 位置控制器 → 姿态控制器 → 动力学
- **灵活配置**：支持NMPC/LinearMPC切换，前馈/自适应可独立开关
- **性能优化**：前馈控制提升响应速度，LinearMPC降低计算负担
...
| Phase 12 | 多机协同与安全滤波（联合 NMPC、CBF-QP、编队控制） | 计划中 |
...
│  QP 实时滤波保证前向不变性                        │
├─────────────────────────────────────────────────┤
│    单机 MPC 控制层（NMPC/LinearMPC 可切换）      │
│  • NMPC: 非线性优化，高精度                      │
│  • LinearMPC: 线性化QP，低计算量（-50~70%）     │
│  • 输出: u*[0] → 姿态+推力                       │
├─────────────────────────────────────────────────┤
│    姿态控制器（PID + 前馈 + 自适应）             │
│  • 前馈控制: 减少延迟，响应速度 +20~30%          │
│  • 自适应控制: 补偿扰动，稳态误差 -50~70%        │
└─────────────────────────────────────────────────┘
```
